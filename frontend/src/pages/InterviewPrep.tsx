...
export default function InterviewPrep({ onBack }: { onBack: () => void }) {
  ...
  const generate = async () => {
    ...
    setError(""); setLoading(true); setResult(null); setOpenAnswers(new Set());
    try {
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      doc.text(`Type: ${result.interviewType}  ·  ${result.questions.length} Questions  ·  Generated by ResumeIQ`, margin, y); y += 12;
      ...
      result.questions.forEach((q, i) => {
        ...
        doc.text(`[${typeLabel}]`, margin, y); y += 5;
        ...
        doc.text("Key Points to Cover:", margin + 2, y); y += 4.5;
        ...
        doc.text(tipLines, margin + 2, y); y += tipLines.length * 4 + 10;
      });

      doc.save(`InterviewPrep_${result.jobRole.replace(/\s+/g, "_")}.pdf`);
    } catch (e) {
    ...
    <div className="mx-auto max-w-3xl px-5 pb-20 pt-4">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
          Get tailored interview questions with model answers and coaching tips.
        </p>
      </motion.div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <select
              className={inputCls}
              value={questionCount}
              onChange={e => setQuestionCount(Number(e.target.value) as 5 | 7 | 10)}
            >
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <span className="ml-2 text-xs text-slate-500">for {result.jobRole} · {result.interviewType}</span>
              </div>
              <button
            ...
            <div className="space-y-3">
              {result.questions.map((q, i) => {
                ...
                const badgeCls = TYPE_BADGE[typeKey] ?? TYPE_BADGE["technical"];
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`mb-1.5 inline-block rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${badgeCls}`}>
                          {typeKey}
                        </span>
                      ...
                      {isOpen && (
                        ...
                        >
                          <div className="border-t border-slate-200 dark:border-slate-700 px-4 pb-5 pt-4 space-y-4">
                            {/* Key points */}
                            <div>
                              <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
                                Key Points to Cover
                              </p>
                            ...
                            {q.tip && (
                              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-700/50 dark:bg-amber-900/20">
                                <p className="text-xs text-amber-800 dark:text-amber-300">
                                  <span className="font-bold">💡 Coach Tip: </span>{q.tip}
                                </p>
                              </div>
                            )}
                          </div>
